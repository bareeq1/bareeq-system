...
function App() {
  ...
  useE(() => {
    let active = true;
    apiFetch('/catalog').then(data => active && setCatalog(mapCatalog(data))).catch(() => {});
    apiFetch('/subscriptions/plans').then(data => active && setPlans(data || [])).catch(() => {});
    return () => {
      active = false;
    };
  }, []);
  useE(() => {
    try {
      if (token) localStorage.setItem('bareeq.token', token);else localStorage.removeItem('bareeq.token');
    } catch {}
  }, [token]);
  useE(() => {
    let active = true;
    ...
    apiFetch('/me', {}, token).then(data => active && setUser(data)).catch(() => {});
    apiFetch('/loyalty/summary', {}, token).then(data => active && setLoyalty(data)).catch(() => {});
    apiFetch('/orders', {}, token).then(data => active && setOrders(data || [])).catch(() => {});
    apiFetch('/favorites', {}, token).then(data => active && setFavorites(data || [])).catch(() => {});
    apiFetch('/addresses', {}, token).then(data => active && setAddresses(data || [])).catch(() => {});
    return () => {
      active = false;
    };
  }, [token]);
  ...
  const addressList = useM(() => addresses.map(a => ({
    id: a.id,
    label: a.label,
    detail: a.detail,
    primary: a.isPrimary
  ...
  const subscriptionPlans = useM(() => plans.map(plan => ({
    id: plan.id,
    label: plan.label,
    price: plan.price,
    ...
    const client = window.google.accounts.oauth2.initCodeClient({
      ...
      callback: async response => {
        ...
        try {
          ...
          setUser(data.user);
          setLoginOpen(false);
          if (data.user?.role === 'Admin') setScreen('admin-dashboard');else if (data.user?.role === 'BranchStaff') setScreen('kds-board');
        } catch (err) {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, tr('nav.language'))), token ? /*#__PURE__*/React.createElement("button", {
    className: "icon-btn",
    ...
    go: setScreen,
    token: token
  }), screen === 'admin-dashboard' && /*#__PURE__*/React.createElement(AdminDashboardScreen, {
    go: setScreen,
    token: token,
    user: user
...
function LoginModal({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Sign in to earn sparkles, save favorites, and track your orders.")), /*#__PURE__*/React.createElement("button", {
    onClick: onGoogle,
    disabled: busy,
    style: {
      width: '100%',
      ...
      background: busy ? 'var(--ivory-2)' : 'var(--paper)',
      fontSize: 14,
      fontWeight: 500,
    ...
    onMouseEnter: e => {
      if (!busy) e.currentTarget.style.borderColor = 'var(--ink)';
    },
    onMouseLeave: e => {
