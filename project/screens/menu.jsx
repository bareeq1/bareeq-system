    ...
    <div className="screen">
      ...
      <section style={{ background: 'var(--burgundy)', color: 'var(--ivory)', padding: '50px 0 60px', position: 'relative' }} className="grain">
        <div className="wrap-wide" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', alignItems: 'end', gap: 60 }}>
          <div>
            <Eyebrow gold style={{ color: 'var(--gold)' }}>{t('menu.kicker')}</Eyebrow>
            <h1 className="serif" style={{ margin: '18px 0 0', fontSize: 96, lineHeight: 0.96, letterSpacing: '-0.02em', fontWeight: 400 }}>
              {t('menu.title1')}<br/>
              <em style={{ color: 'var(--gold)' }}>{t('menu.title2')}</em>
            </h1>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, alignItems: 'flex-end' }}>
            <div className="arabic" style={{ fontSize: 32, color: 'var(--gold)', textAlign: 'right' }}>
              {lang === 'en' ? 'ثمانية وأربعون سببًا للعودة' : 'Forty-eight reasons to come back'}
            </div>
            <div className="mono" style={{ color: 'rgba(255,240,225,0.6)', fontSize: 11, letterSpacing: '0.2em', textAlign: 'right' }}>
              {t('menu.edition')}<br/>HELWAN · 4 — 7 — 2026
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="wrap" style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: '18px 32px' }}>
          <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
            ...
            <select value={sort} onChange={e => setSort(e.target.value)} style={{ padding: '12px 16px', borderRadius: 999, border: '1px solid var(--rule)', background: 'var(--paper)', fontSize: 13 }}>
              <option value="curated">{t('menu.sortCurated')}</option>
              <option value="low">{t('menu.sortLow')}</option>
              <option value="high">{t('menu.sortHigh')}</option>
              <option value="cal">{t('menu.sortCal')}</option>
            </select>
          </div>
              ...
              <div key={cat} style={{ marginBottom: 80 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 24 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 18 }}>
                    ...
                    <h3 className="serif" style={{ fontSize: 44, margin: 0, letterSpacing: '-0.015em' }}>
                      {lang === 'ar' ? c.ar : c.label}
                    </h3>
                    <span className={lang === 'ar' ? 'serif' : 'arabic'} style={{ fontSize: 28, color: 'var(--ink-mute)' }}>
                      {lang === 'ar' ? c.label : c.ar}
                    </span>
                  </div>
                  <div className="mono" style={{ fontSize: 11, color: 'var(--ink-faint)', letterSpacing: '0.18em' }}>
                    {String(catItems.length).padStart(2,'0')} {t('common.items')}
                  </div>
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: 600 }}>
        ...
        <div style={{ padding: 36, display: 'flex', flexDirection: 'column', gap: 22, overflow: 'auto' }}>
          <div>
            ...
            <div className="arabic" style={{ fontSize: 22, color: 'var(--ink-mute)', direction: 'rtl', textAlign: 'left' }}>
              {lang === 'ar' ? item.name : item.ar}
            </div>
          ...
          {/* Ingredients */}
          <div>
            <Eyebrow>{t('product.ingredients')}</Eyebrow>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 12 }}>
              {item.desc.split(/[,·]/).map(s => s.trim()).filter(Boolean).map(s => (
          ...
          {/* Notes */}
          <div>
            <Eyebrow>{t('product.notes')}</Eyebrow>
            <input value={notes} onChange={e => setNotes(e.target.value)} placeholder={t('product.notesPlaceholder')} style={{ width: '100%', marginTop: 10, padding: '12px 14px', borderRadius: 12, border: '1px solid var(--rule)', background: 'var(--paper)', fontSize: 13 }} />
          </div>
          ...
          {/* Pairings */}
          <div>
            <Eyebrow gold>{t('product.pairs')}</Eyebrow>
            <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
              {pairs.map(p => (
                <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, padding: 10, borderRadius: 12, border: '1px solid var(--rule)' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div className="serif" style={{ fontSize: 14, lineHeight: 1.2 }}>{lang === 'ar' ? p.ar : p.name}</div>
          ...
          <div style={{ position: 'sticky', bottom: -36, background: 'var(--paper)', paddingTop: 16, marginTop: 'auto', display: 'flex', gap: 14, alignItems: 'center', borderTop: '1px solid var(--rule)', paddingBottom: 0 }}>
            ...
            <button className="btn btn--ink btn--block" onClick={() => { onAdd({ ...item, size, milk, extras, qty, notes, finalPrice: total / qty }); onClose(); }} style={{ flex: 1 }}>
              {t('product.addToCart')} <span style={{ marginLeft: 'auto', opacity: 0.8, fontFamily: 'var(--f-mono)' }}>{t('common.currency')} {total.toFixed(0)}</span>
            </button>
          </div>
        </div>
      </div>
    ...
    <div style={{
      ...
      <aside style={{
        ...
        <div style={{ padding: '22px 26px', borderBottom: '1px solid var(--rule)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <Eyebrow gold>{t('cart.eyebrow')}</Eyebrow>
            <h3 className="serif" style={{ margin: '6px 0 0', fontSize: 26 }}>{t('cart.title')}</h3>
          </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '20px 26px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          {cart.length === 0 && (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--ink-mute)' }}>
              ...
              <div style={{ marginTop: 12 }}>{t('common.empty')}</div>
              <button className="btn btn--ghost btn--sm" style={{ marginTop: 18 }} onClick={() => { onClose(); go('menu'); }}>{t('common.browse')}</button>
            </div>
          )}
          {cart.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 }}>
                  ...
                  <button onClick={() => setCart(c => c.filter((_, idx) => idx !== i))} style={{ fontSize: 11, color: 'var(--ink-mute)', textDecoration: 'underline' }}>{t('common.remove')}</button>
                </div>
              </div>
            </div>
          ))}
        ...
        {cart.length > 0 && (
          <div style={{ padding: '20px 26px', borderTop: '1px solid var(--rule)', background: 'var(--ivory)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: 'var(--ink-soft)', marginBottom: 14 }}>
              <span>{t('cart.delivery')}</span><span className="mono">{t('cart.free')} · ✦{points}</span>
            </div>
