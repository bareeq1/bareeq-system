import { buildGoogleOAuthUrl } from '../api.js';

export default function LoginPage() {
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        <h1 style={styles.title}>Bareeq POS</h1>
        <p style={styles.sub}>Branch staff dashboard</p>
        <button style={styles.btn} onClick={() => { window.location.href = buildGoogleOAuthUrl(); }}>
          <GoogleIcon />
          Sign in with Google
        </button>
      </div>
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" style={{ marginRight: 8 }}>
      <path fill="#4285F4" d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844c-.209 1.125-.843 2.078-1.796 2.717v2.258h2.908c1.702-1.567 2.684-3.875 2.684-6.615z"/>
      <path fill="#34A853" d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.859-3.048.859-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18z"/>
      <path fill="#FBBC05" d="M3.964 10.71A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332z"/>
      <path fill="#EA4335" d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.958L3.964 7.29C4.672 5.163 6.656 3.58 9 3.58z"/>
    </svg>
  );
}

const styles = {
  page: {
    minHeight: '100vh', display: 'flex',
    alignItems: 'center', justifyContent: 'center',
    background: '#fafaf9',
  },
  card: {
    background: '#fff', padding: 40, borderRadius: 16,
    boxShadow: '0 4px 24px rgba(0,0,0,0.08)',
    display: 'flex', flexDirection: 'column',
    alignItems: 'center', gap: 16, minWidth: 300,
  },
  title: { fontSize: 28, fontWeight: 700, letterSpacing: '-0.02em' },
  sub: { fontSize: 14, color: '#78716c', marginTop: -8 },
  btn: {
    display: 'flex', alignItems: 'center',
    padding: '10px 24px', borderRadius: 8,
    border: '1px solid #e7e5e4', background: '#fff',
    cursor: 'pointer', fontWeight: 600, fontSize: 14,
    color: '#1c1917', marginTop: 8,
  },
};
